...
const timerButtonComp = document.getElementsByTagName('timer-buttons')[0];
let isFailed = false;
...
function timerPageInit() {
    ...
    const id = localStorage.getItem('currentTask');
    ...
    if (localStorage.getItem('shortBreak') === 'true') {
        ...
        timerButtonComp.distractionCount()
...
function displayBreakComplete() {
    ...
    timerButtonComp.renderBreakCompleteModal(true)
}
...
function autoContinue() {
    if (localStorage.getItem('shortBreak') === 'true') {
        ...
        setTimeout(() => {
            timerButtonComp.continueTask();
            timerButtonComp.startTimer();
        }, 5500);
    }
}
...
function displayBreak() {
    timerButtonComp.renderDisplayBreak()
    ...
    setTimeout(() => {
        ...
        // hideButtons();
        timerButtonComp.renderStartButton(false);
        timerButtonComp.renderStartBreakButton(true, false)
    }, 2000);
}
...
function startBreak() {
    // Making the start break button the only one visible
    timerButtonComp.renderStartBreakButton(true, true)
    if (localStorage.getItem('shortBreak') === 'true') {
        start();
    } else {
        start();
